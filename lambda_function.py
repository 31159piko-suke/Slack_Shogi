...
def update(respond, body, client):
    ...
    if user == teban_user:
        try:
            ...
            display_sashite = input.display(parsed_sashite)
            ...
            try:
                ...
                client.chat_update(
                    ...
                    blocks=generate_board_block(
                        url, status, teban_user, tesu=tesu, sashite=display_sashite
                    ),
                    text=f"{tesu}手目 {display_sashite}",
                )
    else:
...
def lose(respond, body, client):
    ...
    input = Input()
    display_sashite = input.display(last_sashite)
